package com.wang.config.swagger;
...
@Configuration
@EnableSwagger2
@EnableConfigurationProperties(SwaggerProperties.class)// 读取自定义的配置类
public class SwaggerAutoConfiguration {

    private SwaggerProperties swaggerProperties;// 注入配置类
    ...
    public Docket docket() {
        Docket docket = new Docket(DocumentationType.SWAGGER_2)
                .apiInfo(apiInfo())
                .select()
                .apis(RequestHandlerSelectors.basePackage(swaggerProperties.getBasePackage()))// 对哪个包生成swagger文档
                .paths(PathSelectors.any())// 对哪个路径，即对controller的所有路径
                .build();
        // 安全的配置：因为本身要作为资源服务器，所以进行必要的安全配置
