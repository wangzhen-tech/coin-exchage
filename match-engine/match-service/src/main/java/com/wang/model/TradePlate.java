...
public class TradePlate {
    ...
    public void add(Order order) {
        ...
        for (i = 0; i < items.size(); i++) {
            ...
            if (    // 买入时比当前的价格小，那么continue，因为要排在当前位置的后边；卖出时相反
                ...
                // 还不能插入,往前走一步
                continue;
            } else if (depthItemVo.getPrice().compareTo(order.getPrice()) == 0) {// 两者价格相同
    ...
    public void remove(Order order, BigDecimal amount) {
        ...
        while (iterator.hasNext()) {
            ...
            if (order.getPrice().compareTo(next.getPrice()) == 0) { // 价格相同
                next.setVolume(next.getVolume().subtract(amount));// 减去成交的数量
                if (next.getVolume().compareTo(BigDecimal.ZERO) <= 0) {
                    iterator.remove(); // 减完之后数量为0了，直接可以摘掉这个盘口数据
                }
            }
